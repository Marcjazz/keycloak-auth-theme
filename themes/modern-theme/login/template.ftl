...
<head>
    <meta charset="UTF-8" />
    <title><#if realm.displayName??>${kcSanitize(realm.displayName)?no_esc}<#else>Keycloak</#if> - <#nested 'title'></title>
...
<#-- Default bodyClass, can be overridden by specific pages -->
<#macro template title="" bodyClass="" scripts=[]>
<#assign bodyClass = bodyClass!"flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-4">
<body class="${bodyClass}">
    <#-- Main content card structure, common to login pages -->
    <div class="w-full max-w-md p-6 sm:p-8 space-y-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl">
        <header class="space-y-2 text-center">
            <#if properties.logoUrl?has_content>
                <img src="${properties.logoUrl}" alt="${kcSanitize(realm.displayName!'Keycloak')?no_esc} Logo" class="h-12 mx-auto" loading="lazy">
            <#-- Fallback logo can be added here if desired -->
            </#if>
    ...
    <#-- Include main.js if it exists and is needed globally for login theme -->
    <#-- <script src="${url.resourcesPath}/js/main.js" defer></script> -->
</body>
</html>
</#macro>

<#-- Define a macro for pages to use this layout -->
<#macro mainLayout title="" header="" bodyClass="" scripts=[]>
    <@template title=title bodyClass=bodyClass scripts=scripts>
